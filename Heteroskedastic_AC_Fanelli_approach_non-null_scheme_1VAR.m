...
load 'Material for Assignment 2-20231201'\DATI_FISCAL_CK.txt;
...
VAR_Const = [NaN; NaN; NaN; NaN]; %setting the vector of the parameters associated with th constant
 ...
 Pi1 =[NaN NaN NaN NaN; 
       NaN NaN NaN NaN;
       NaN NaN NaN NaN;
       NaN NaN NaN NaN] ;      % Each matrix is a 3x3 and will collect the parameters for each lag
...
%% Identification scheme
 ...
 rng(9) %the seed that guarantees to find reasonable structural parameters 
  ...
  C =[NaN  0 NaN  0; 
   ...
   NaN NaN  NaN   0;
   0 NaN NaN  NaN];
ParamNumberA=[ 1, 3,  6, 7, 8, 9,10, 11, 12, 16]';
ParamNumberC=[ 1, 3,  6, 7, 8, 9,10, 11, 12, 16]';
...
for c_par= 1:  size(ParamNumberA,1)
     ...
     SE_A(ParamNumberA(c_par,1))= SE_Hessian_MATRIX(c_par); %filling the stanard errors matrix
...
for c_par= 1:  size(ParamNumberC,1)
     ...
     SE_C(ParamNumberC(c_par,1))= SE_Hessian_MATRIX(c_par+size(ParamNumberA,1)); %filling the stanard errors matrix
...
A
AC=(A+C)
A_inv=inv(A)    %this section helps in looking at the matrices before the sign normalization
AC_inv=inv(AC)
%% Sign normalization  for A and A+C along the main diagonal
    if A(1,1)<0
    A(:,1)=-A(:,1);
    end
    if A(2,2)<0
...
rank(Jacobian)                                      %if the two numbers match, then the paramters estimated are locally identified
...
for boot=1:BootstrapIterations
    ...
    Res_star_1=MMB_res(Residuals_preV, T_1, 4, M); %applying the moving block bootstrap function to the original residuals
    Res_star_2=MMB_res(Residuals_GM, T_2, 4, M);
    Residuals_Boot=[Res_star_1;Res_star_2];


 DataSet_Bootstrap=zeros(T_overall+p,M); %the first p obersevations are settet to be zero as suggested in Bruggenam et al
      ...
      DataSet_Bootstrap=DataSet_Bootstrap(end-T_overall+1:end,:); %cutting the first p obervations 


 T_1_boot=(T_break-p-p);
 T_2_boot=((T_overall-p)-(T_break-p-p));

%unlogged_gdp_boot= exp(DataSet_Bootstrap(:,3)); 
%unlogged_tax_boot=exp(DataSet_Bootstrap(:,1));        %obtiaining a
                                                                     %bootstraped version of the average ratios 
%unlogged_g_boot=exp(DataSet_Bootstrap(:,2));

%Dynamic_tax_multiplier_ratio_1_boot=mean(unlogged_gdp_boot(1:T_1_boot+p,1)./unlogged_tax_boot(1:T_1_boot+p,1));
%Dynamic_tax_multiplier_ratio_2_boot=mean(unlogged_gdp_boot(T_2_boot+p:end,1)./unlogged_tax_boot(T_2_boot+p:end,1));%average of the unlogged gdp and unlogged tax ratio

%FiscalSp_multiplier_ratio_1_boot=mean(unlogged_gdp_boot(1:T_1_boot+p,1)./unlogged_g_boot(1:T_1_boot+p,1));
%FiscalSp_multiplier_ratio_2_boot=mean(unlogged_gdp_boot(T_2_boot+p:end,1)./unlogged_g_boot(T_2_boot+p:end,1));
...
[EstVAR_Boot,EstSE_Boot,logLikVAR_Boot,Residuals_Boot] = estimate(VAR,DataSet_Bootstrap); 
 ...
 %First and second period bootstraped residuals and variance-covariance
 %matrix
 ...
 Initial_AC_boot=StructuralParam_Estimation_MATRIX'; %the initial value of the algorithm optimization procedure is setted to be the vectorized structural parameters matrix in order to guarantee the same local identification for each bootstrap repetition
...
% Sign normalization  for A+C   
    ...
    A_boot_inv=inv(A_boot);
    AC_boot_inv=inv(AC_boot);
   ...
   J=[eye(M) zeros(M,M*(p-1))]; %the selection matrix for the companion form
    ...
    for h = 0 : HorizonIRF
    TETA_Boot_1(:,:,h+1,boot)=J*CompanionMatrix_Boot^h*J'*A_boot_inv;
    TETA_Boot_2(:,:,h+1,boot)=J*CompanionMatrix_Boot^h*J'*AC_boot_inv; %obtaining the impulse response functions(.,.) for each bootstrapped sample in two different periods
    end 
    ...
    Impulse_tax_to_GDPh_boot_1= e_j'*J*CompanionMatrix_Boot^h*J'*A_boot_inv*e_i; %getting the impulse response of taxes on GDP for each of the 20 steps ahead
    Impulse_tax_to_GDPh_boot_2= e_j'*J*CompanionMatrix_Boot^h*J'*AC_boot_inv*e_i;
    
     
    Impulse_tax_to_TAX_boot_1=e_i'*A_boot_inv*e_i; %getting the on impact response of a shock in taxes on taxes
    Impulse_tax_to_TAX_boot_2=e_i'*AC_boot_inv*e_i;
    
    Impulse_g_to_GDPh_boot_1=e_j'*J*CompanionMatrix_Boot^h*J'*A_boot_inv*b_i; %getting the impulse response of fiscal spending on GDP for each of the 20 steps ahead 
    Impulse_g_to_GDPh_boot_2=e_j'*J*CompanionMatrix_Boot^h*J'*AC_boot_inv*b_i;
    
    Impulse_g_to_g_boot_1=b_i'*A_boot_inv*b_i; %getting the on impact response of a shock in fiscal spending  on fiscal spending
    Impulse_g_to_g_boot_2=b_i'*AC_boot_inv*b_i;
...
%% IRFs for the great moderation era
...
CompanionMatrix = [mP1 mP2 mP3 mP4;           
    ...
    Impulse_tax_to_GDPh_2=e_j'*J*CompanionMatrix^h*J'*C_IRF_2*e_i;%properly selecting the shocks as did for the bootstrap case (here the multipliers are been computed on the whole sample to obtain the point estimates)
