...
%% Global Variables 
global p 
global Likelihood_SVAR2% # of VAR lags
global T_overall  
global T_1 
...
global M 
global T_break %where the break date is positioned in the original dataset
...
p=4;
T_break=136;
T_overall=size(DATI_FISCAL_CK,1)-p;
...
Var_dataset0=[DATI_FISCAL_CK(1:end,Variablesselection),RR(1:end,:)];
Var_dataset1=[DATI_FISCAL_CK(1:T_break,Variablesselection),RR(1:T_break,:)];
Var_dataset2=[DATI_FISCAL_CK(T_break+1:end,Variablesselection),RR(T_break+1:end,:)];

 %% VAR  overall settings
...
%% Estimates for the overall period
[EstVAR0,EstSE0,logLikVAR0,Residuals_pre0] = estimate(VAR,Var_dataset0); % Estimation of the above specified var
...
%% Estimates for the first period 

[EstVAR1,EstSE1,logLikVAR1,Residuals_preV] = estimate(VAR,Var_dataset1); % Estimation of the above specified var
...
%% VAR estimation for Great_M
 



[EstVAR2,EstSE2,logLikVAR2,Residuals_GM] = estimate(VAR,Var_dataset2); % Estimation of the above specified var
...
%% obtianing the variance-covariance matrices for both periods
T_1=size(Residuals_preV,1);
T_2=size(Residuals_GM,1);


Sigma_u_preV=(Residuals_preV'*Residuals_preV)/T_1;

Sigma_u_GM=(Residuals_GM'*Residuals_GM)/T_2;

Sigma_u_1=Sigma_u_preV;
Sigma_u_2= Sigma_u_GM;

%% Test for structural change of all 80 parameters 
LR=-2*(logLikVAR0-(logLikVAR1+logLikVAR2));
...
[StructuralParam_Estimation_MATRIX,Likelihood_SVAR2,exitflag, output, grad,Hessian_MATRIX] = fminunc('Likelihood_Fanelli_Heterosk', Initial_AC', options);
...
Likelihood_SVAR2 = -1*Likelihood_SVAR2; %obtaining a positive likelihood of the SVAR since we are interested in a maximum 
...
Likelihood_SVAR2 = -1*Likelihood_SVAR2;

                                                       
%% generating the selection vectors for the fiscal and the tax multiplier
...
for boot=1:BootstrapIterations
 ...
 DataSet_Bootstrap1=zeros(T_1+p,M);
 DataSet_Bootstrap2=zeros(T_2+p,M);

  
%building two different sets of obervations
       ...
       for t = p+1 : T_2+p
             DataSet_Bootstrap2(t,:)=Const_2 + Trend_2 + mP1_2 * DataSet_Bootstrap2(t-1,:)' +...
                                       ...
                                       mP4_2 * DataSet_Bootstrap2(t-4,:)' + Res_star_2(t-p,:)';
       end


      DataSet_Bootstrap1=DataSet_Bootstrap1(end-T_1+1:end,:);
      DataSet_Bootstrap2=DataSet_Bootstrap2(end-T_2+1:end,:);


%%First var estimation
...
[EstVAR_Boot2,EstSE_Boot2,logLikVAR_Boot2,Residuals_GM_boot] = estimate(VAR, DataSet_Bootstrap2); 
 ...
 % Secon  period bootstrapped variance-covariance matrix
 
 Sigma_u_GM_boot=(Residuals_GM_boot'*Residuals_GM_boot)/T_2_boot;

 
 Sigma_u_1_boot=Sigma_u_preV_boot;    %those two lines are necessary to let these two matrices being recognized in a standard function
 ...
 Initial_AC_boot=StructuralParam_Estimation_MATRIX'; %starting from the previous estimated vectorized structural parameters matrix in order to guarantee a similar maximum for each bootstrap
...
[StructuralParam_Estimation_MATRIX_boot,Likelihood_SVAR_boot,exitflag, output, grad,Hessian_MATRIX] = fminunc('Likelihood_Fanelli_Heterosk_boot', Initial_AC_boot', options);
...
%sign normalization for the A nd A+C matrices
   if A_boot(1,1)<0
    A_boot(:,1)=-A_boot(:,1);
    end
    if A_boot(2,2)<0
...
J=[eye(M) zeros(M,M*(p-1))]; %the selection matrix for the companion form
    CompanionMatrix_Boot1 = [mP1_Boot1 mP2_Boot1 mP3_Boot1 mP4_Boot1;
                            eye(M*(p-1)) zeros(M*(p-1),M)];        %companion matrix for the first bootstrapped VAR

    CompanionMatrix_Boot2 = [mP1_Boot2 mP2_Boot2 mP3_Boot2 mP4_Boot2 ;
                            eye(M*(p-1)) zeros(M*(p-1),M)];     %companio matrix for the second bootstrapped VAR
...
CompanionMatrix1 = [mP1_1 mP2_1 mP3_1 mP4_1;           %  First VAR companion matrix
...
CompanionMatrix2 = [mP1_2 mP2_2 mP3_2 mP4_2;           %  Second VAR companion matrix
                   eye(M*(p-1)) zeros(M*(p-1),M)];
...
J=[eye(M) zeros(M,M*(p-1))]; %selection matrix for the companion form
